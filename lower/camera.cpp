...
bool Camera::process_image(unsigned char *imageSrc,unsigned char * imageDst) {     
    ...
    for(in=0,out=0;in<size;in+=4,out+=6){         
        ...
        y1=imageSrc[in+2];         
        v=imageSrc[in+3];          
        pixel24=cal_yuvtorgb(y0,u,v);         
        imageDst[out+0]=pixel[0];         
        imageDst[out+1]=pixel[1];         
        imageDst[out+2]=pixel[2];          
        pixel24=cal_yuvtorgb(y1,u,v);         
        imageDst[out+3]=pixel[0];         
        imageDst[out+4]=pixel[1];         
        imageDst[out+5]=pixel[2];        
    }     
        return 0; 
}
...
bool Camera::OpenDevice(){          //qt1需要调用的打开摄像头的函数
    if (open_device()) {            //打开摄像头
        printf("open success\n");
        if (init_device()) {        //设置视频格式
//		printf("************** %s, line = %d\n", __FUNCTION__, __LINE__);
            if (start_capturing())
//		printf("************** %s, line = %d\n", __FUNCTION__, __LINE__);
                return true;
